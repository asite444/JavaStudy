...
public class ApocalypseNumber {
	...
	public static void main(String[] args) {
		...
		for(int i=666;;i++) {
			checkNumber=Integer.toString(i);
			for(int j=0;j<=checkNumber.length()-3;j++){
				if(checkNumber.charAt(j)=='6' && checkNumber.charAt(j)==checkNumber.charAt(j+1) && checkNumber.charAt(j+1)==checkNumber.charAt(j+2)) {
					count++;
					break;
				}				
			}
